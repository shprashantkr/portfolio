...
useEffect(() => {
  ...
  return (
    <>
      <div className='fixed top-0 right-0 w-11/12 -z-10 dark:hidden'>
        <Image src={assets.header_bg_color} alt='' className='w-full'/>
      </div>
      <nav
        className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white/50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20 py-0 h-20":"py-4"}`}
       
      >
        <a href="#top">
          <Image
            src={isDarkMode? assets.myLogo_dark :assets.myLogo }
            alt=""
